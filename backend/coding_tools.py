"""Scoped coding helpers for ComfyUI custom node development."""
...
MCP_REPO_ROOT = Path(__file__).resolve().parents[1]
...
def create_pack(
    name: str,
    *,
    node_class: str = "FLMCPExampleNode",
    display_name: str = "FL-MCP Example Node",
    category: str = "FL-MCP",
    ...
    node_class = re.sub(r"[^A-Za-z0-9_]", "", node_class) or "FLMCPExampleNode"
    files = {
...
''',
    ...
    def INPUT_TYPES(cls):
        return {{
            "required": {{
                "text": ("STRING", {{"default": "hello from FL-MCP", "multiline": True}}),
            }}
        }}
